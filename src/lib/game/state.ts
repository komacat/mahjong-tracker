...
export function canPassDealership({
    ...
    const dealerPlacement = state.players.reduce(
        (acc, player) => acc + (player.score > dealer.score ? 1 : 0),
        1
    )

    if (
        !(ruleset.oyaNagashi || (isLastRound && dealerPlacement <= (ruleset.allLastPlacement ?? 0)))
    ) {
        return false
    }
    ...
    const lastAction = actions.findLast(
        (it) =>
            it.type === 'ron' ||
            it.type === 'tsumo' ||
            ((ruleset.oyaNagashi || ruleset.allLast === 'TENPAIYAME') && it.type === 'draw')
    )
...
> {
    ...
    const sortedScores = Object.entries(scores)
        .map<[string, number, number]>(([winner, score]) => {
            ...
            const distance =
                (winnerIndex - loserIndex + state.players.length) % state.players.length
...
export const computeState = wrapCatching(
    ({
        user,
        players,
        ruleset,
        ...
        gameState.update((state) => {
            for (const action of actions) {
                ...
                switch (action.type) {
                    ...
                    case 'ron': {
                        ...
                        for (const player of state.players) {
                            if (!repeat) {
                                player.wind =
                                    (player.wind - 1 + state.players.length) % state.players.length
                            }
                            player.richi = false
                        }
                    ...
                    case 'tsumo': {
                        ...
                        if (
                            ...
                            (ruleset.player === 'FOUR' &&
                                Object.keys(action.scores).length !== 3) ||
                    ...
                    case 'draw': {
                        ...
                        state.history.push({
                            ...
                            result: state.players.map((player) => ({
                                player: player.user.id,
                                score:
                                    (action.tenpai.includes(player.user.id)
                                        ? tenpaiFeeReceived
                    ...
                    case 'chonbo': {
                        ...
                        switch (chonbo.type) {
                            ...
                            case 'custom': {
                                if (affectsScore) {
                                    const score =
                                        player.wind === 0 ? chonbo.dealer : chonbo.nonDealer
                ...
                if (state.round >= endRound) {
                    done = state.players.some(
                        (player) => player.score >= (ruleset.suddenDeath || 0)
                    )
                }

                done ||=
                    !!ruleset.calledGame &&
                    state.players.some((player) => player.score >= ruleset.calledGame!)

                done ||=
                    ruleset.tobi &&
                    state.players.some(
                        (player) => player.score < 0 || (ruleset.tobiAtZero && player.score === 0)
                    )
                ...
                if (done) {
                    ...
                    if (ruleset.uma.type === 'simple') {
                        ...
                        const floating = state.players.reduce(
                            (acc, player) => acc + (player.score > ruleset.startScore ? 1 : 0),
                            0
                        )
