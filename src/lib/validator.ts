...
export function validateUma(uma: any): uma is PrismaJson.Uma {
    ...
    return (
        (uma.type === 'simple' &&
            Array.isArray(uma.uma) &&
            uma.uma.length === 4 &&
            uma.uma.every((x: any) => typeof x === 'number')) ||
        (uma.type === 'floating' &&
            Array.isArray(uma.A) &&
            Array.isArray(uma.B) &&
            Array.isArray(uma.C) &&
...
export function validateChonbo(chonbo: any, scoring: ScoringSheet): chonbo is PrismaJson.Chonbo {
    return (
        (chonbo != null &&
            ...
            chonbo.type === 'score' &&
            typeof chonbo.name === 'string' &&
            scoring.dealer.tsumo.some(([name]) => name === chonbo.name)) ||
        (chonbo.type === 'fixed' &&
            typeof chonbo.point === 'number' &&
            chonbo.point >= 0 &&
            chonbo.point % 100 === 0) ||
        (chonbo.type === 'custom' &&
            typeof chonbo.dealer === 'object' &&
            typeof chonbo.nonDealer === 'object' &&
            ...
            chonbo.dealer.toDealer === 0 &&
            chonbo.dealer.toNonDealer >= 0 &&
            chonbo.nonDealer.toDealer >= 0 &&
            chonbo.nonDealer.toNonDealer >= 0 &&
            chonbo.dealer.toNonDealer % 100 === 0 &&
            chonbo.nonDealer.toDealer % 100 === 0 &&
            chonbo.nonDealer.toNonDealer % 100 === 0)
    )
}
...
function checkObjectSN(
    object: any,
    max: number | undefined = undefined
): object is { [key: string]: number } {
    return (
        object != null &&
        typeof object === 'object' &&
        Object.entries(object).every(([k, v]) => typeof k === 'string' && v != 'number') &&
    ...
    return (
        (action != null &&
            ...
            ((action.type === 'riichi' && typeof action.player === 'string') ||
                ...
                (action.type === 'draw' &&
                    Array.isArray(action.tenpai) &&
                    action.tenpai.every((x: any) => typeof x === 'string')) ||
                (action.type === 'nagashi' &&
                    Array.isArray(action.players) &&
                    action.players.every((x: any) => typeof x === 'string')) ||
                action.type === 'oyaNagashi' ||
                (action.type === 'ron' &&
                    typeof action.loser === 'string' &&
                    typeof action.scores === 'object' &&
                    Object.entries(action.scores).every(
                        ([k, v]) => typeof k === 'string' && typeof v === 'number'
                    )) ||
                (action.type === 'tsumo' &&
                    typeof action.winner === 'string' &&
                    typeof action.scores === 'object' &&
                    Object.entries(action.scores).every(
                        ([k, v]) => typeof k === 'string' && typeof v === 'number'
                    )) ||
                (action.type === 'start' &&
                    ...
                    DateTime.fromISO(action.at).isValid) ||
                (action.type === 'resume' &&
                    typeof action.at === 'string' &&
                    DateTime.fromISO(action.at).isValid) ||
                (action.type === 'pause' &&
