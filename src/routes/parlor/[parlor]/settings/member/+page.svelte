...
<main class="mx-auto max-w-2xl">
	...
	<section>
		<h2 class="p-4 text-xl font-semibold">
			Members ({data.member.filter(({ status }) => status === 'ACCEPTED').length})
		</h2>
		{#each data.member.filter(({ status }) => status === 'ACCEPTED') as member}
			<div class="flex flex-row items-center justify-between space-x-4 p-4">
				<UserAvatar user={member.user} />
				<p class="flex-grow">{member.user.username}</p>
				<button
					on:click={() => decision(member.user.id, 'remove')}
	...
	<section>
		<h2 class="p-4 text-xl font-semibold">
			Pending Join Requests ({data.member.filter(({ status }) => status === 'PENDING').length})
		</h2>
		{#each data.member.filter(({ status }) => status === 'PENDING') as joinRequest}
